...
class Template {
	...
	function display_dashboard($template,$data=null)
	{
		$data['_content'] = $this->_ci->load->view($template ,$data, TRUE);


		$data['_includes'] = $this->_ci->load->view('includes' ,$data, TRUE);
		$data['_header'] = $this->_ci->load->view('header_dinkes');
		$data['_side_menu'] = $this->_ci->load->view('side_menu_dinkes' ,$data, TRUE);
		$data['_footer'] = $this->_ci->load->view('footer' ,$data, TRUE);


		$this->_ci->load->view('/index_dinkes.php', $data);
	}
	
	function display_dash_walikota($template,$data=null)
	{
		$data['_content'] = $this->_ci->load->view($template ,$data, TRUE);


		$data['_includes'] = $this->_ci->load->view('includes' ,$data, TRUE);
		$data['_header'] = $this->_ci->load->view('header_walikota');
		$data['_side_menu'] = $this->_ci->load->view('side_menu_walikota' ,$data, TRUE);
		$data['_footer'] = $this->_ci->load->view('footer' ,$data, TRUE);


		$this->_ci->load->view('/index_walikota.php', $data);
	}
...
/* -------------------- TAMPIL STOK AWAL GUDANG --------------------------*/
    function tampil_sawal_gudang($template,$data=null)
    {

        $this->_ci->load->view('/daftar_sawal_gudang.php', $data);
    }
...
////////////////*  CARI bayar tindakan *//////////////////	
	function tampilbayartindakan($template,$data=null)
	{
		
		$this->_ci->load->view('ListBayarTindakan.php', $data);
	}
...
///////////*  CETAK NOTA  *//////////////
	...
	function tampil_cetak_antrian($template,$data=null)
    {

        $this->_ci->load->view('cetak/cetak_antrian.php', $data);
    }
