<style type="text/css">
	...
	#rujuk { display: none;}
	#rujuk-rs { display: none;}
	#rujuk-poli {display: none;}
...
<div aria-hidden="false" aria-labelledby="DataRekamMedisLabel" role="dialog" tabindex="-1" class="modal hide fade in" id="DataRekamMedis">
        <div class="modal-header">
            <button aria-hidden="true" data-dismiss="modal" class="close" type="button">×</button>
            <h3 id="DataRekamMedisLabel">Cari Data Pasien</h3>
        </div>
        <div class="modal-body">
            <div id='loading' style='display:none;'>
                <span class="img-responsive">Memuat data ... <img src='http://localhost/simpus-tansa/assets/img/loaders/loader6.gif'/></span>
            </div>
            <p class="content">
            	...
            	<div class="span12">
					...
					<input type="hidden" name="tanggal_lahir" id="tanggal_lahir2" class="input-small" value="<?php echo $this->functions->convert_date_indo(array("datetime" => $edit_pasien['tanggal_lahir'])); ?>" readonly />
                            ...
                            <div class="span6">
                            	<table class="table table-bordered table-invoice">
									<tr> <td> Tanggal</td>
									  <td> <input type="text" name="tgl_pelayanan" id="tgl_pelayanan2" class="input-small" value="<?php echo $this->functions->convert_date_indo(array("datetime" => $row['tgl_pelayanan'])); ?>" /></td>
									  </tr>
                             <!--         <tr>
                                        <td>NIK</td>
                        ...
                        <script type="text/javascript">
							jQuery(document).ready(function () {
                                ...
                                setInterval('$("#dyntable").dataTable().fnDraw()', 5000);

                            });

                        </script>
                        <div class="clearfix" style="margin-bottom:20px"></div>
                        <table style="auto;width:40%;border-collapse:collapse;background:#ecf3eb">
                            <tr>
                                <td colspan="2"><h4>Keterangan Status Warna</h4></td>
                            </tr>
                            <tr>
                                <td align="center" style="height: 40px"><button class="btn btn-warning btn-circle" title="BELUM DILAYANI"><i class="iconsweets-wifi2 iconsweets-white"></i></button></td><td>Pasien <b>BELUM</b> diLayani</td>
                            </tr>
                            <tr>
                                <td align="center" style="height: 40px"><button class="btn btn-danger btn-circle" title="SEDANG DILAYANI"><i class="iconsweets-wifi2 iconsweets-white"></i></button></td><td>Pasien <b>SEDANG</b> diTangani</td>
                            </tr>
                            <tr>
                                <td align="center" style="height: 40px"><button class="btn btn-primary btn-circle" title="SUDAH DILAYANI"><i class="iconsweets-wifi2 iconsweets-white"></i></button></td><td>Pasien <b>SUDAH</b> diLayani</td>
                            </tr>
                            <tr>
                                <td align="center" style="height: 40px"><button class="btn btn-success btn-circle" title="DIRUJUK KE TEMPAT LAIN"><i class="iconsweets-wifi2 iconsweets-white"></i></button></td><td>Pasien <b>DIRUJUK</b> ke Tempat Lain</td>
                            </tr>
                        </table>
                        </div>
						...
						<button type="button" name="list_medis" id="list_medis" class="btn btn-danger btn-small" title="Cari No Rekam Medis"><i class="icon-list-alt icon-white"></i></button>
                            ...
                            <div class="span6">
                            	<table class="table table-bordered table-invoice">	
                                <tr> <td>Tanggal</td>
								<td><input type="text" name="tgl_pelayanan" id="tgl_pelayanan"  class="input-small" value="<?php echo date('d-m-Y'); ?>" /></td>
	                                   </tr>
			<!--	    <tr>
                                        <td>NIK</td>
										...
										<div id="rujuk">
											...
											<div id="rujuk-poli">
											 <p>
												<label>Nama Poli Rujukan</label>
												<span class="field">
													<select name="poli_rujukan" id="poli_rujukan" class="uniformselect">
														<option value="-">Pilih Unit Pelayanan</option>
														<?php foreach($list_unit_pelayanan as $lup) : ?>
															<?php
															if($lup['kd_unit_pelayanan'] == $row['kd_unit_pelayanan'])
...
jQuery(document).ready(function(){
	...
	<?php if(strlen($this->uri->segment(3)) >= 6): ?>
	...
	jQuery('#kd_status_pasien').change(function () {
		...
		else if(jQuery(this).val() == 'SKP-3') // Di rujuk poli
		{
			jQuery('#rujuk').show();
			jQuery('#rujuk-poli').show();
			jQuery('#rujuk-rs').hide();
		}
		else if(jQuery(this).val() == 'SKP-4') // Di rujuk RS
		{
			jQuery('#rujuk').show();
			jQuery('#rujuk-poli').hide();
			jQuery('#rujuk-rs').show();
		}
	});
...
function DataRekamMedis(){
	...
	*/
	...
	jQuery("#list_medis").click(function(e){
		e.preventDefault();
		jQuery("#DataRekamMedis").modal("show");
	});
	
	
	jQuery("#DataRekamMedis").on("show", function(){
		jQuery("#loading").show();
		var html = '<table id="dyntable2" class="table table-bordered"><thead><tr><th>Kode Rekam Medis</th><th>Nama Pasien</th><th>Tanggal Lahir</th><th>Alamat</th><th>Pilih</th></tr></thead></table>';
		jQuery(".modal-body").html(html);
		
		var oTable2 = jQuery('#dyntable2').dataTable({
        	"bProcessing": true,
            "aaSorting" : [[0, 'desc']],
			"bServerSide": true,
			"bAutoWidth": false,
			"sAjaxSource": '<?php echo base_url(); ?>datatable_master/cari_pasien',
			"bJQueryUI": false,
			"sPaginationType": "full_numbers",
			"fnDrawCallback": function(oSettings) {
				jQuery.uniform.update();
	...
	jQuery("#DataRekamMedis").on("shown", function(){
		jQuery("#loading").hide();
	});
	
	function pilih(id){
        jQuery('#DataRekamMedis').modal('hide');
		jQuery("#kd_rekam_medis").val(id);
		jQuery("#kd_rekam_medis").focus();
	}
