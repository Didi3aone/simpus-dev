...
class M_register_harian extends CI_Model {
	...
	function get_pasien_rawat_umum_by_date($tgl_mulai, $tgl_akhir, $kd_unit_pelayanan, $kd_bayar){
        ...
        $sql = $this->db->select('pelayanan.kd_rekam_medis, dokter.nm_dokter,pelayanan.kd_trans_pelayanan, pelayanan.tgl_pelayanan, pasien.no_reg, pelayanan.kd_rekam_medis, pasien.nm_lengkap, pasien.kd_jenis_kelamin, golongan_umur.gol_umur,cara_bayar.cara_bayar, pasien.no_asuransi, unit_pelayanan.nm_unit, set_wil_kerja_pusk.kd_wil, pasien.alamat,kelurahan.nm_kelurahan, kota.nm_kota, pelayanan_penyakit.kd_penyakit,jenis_kasus.jenis_kasus, GROUP_CONCAT(DISTINCT jenis_kasus.jenis_kasus SEPARATOR "; ") as jns_kasus, pelayanan_tindakan.kd_produk, GROUP_CONCAT(DISTINCT tindakan.produk SEPARATOR "; ") as tindakan, status_keluar_pasien.keterangan,GROUP_CONCAT(DISTINCT pelayanan_penyakit.kd_penyakit SEPARATOR "; ") as kd_icd, GROUP_CONCAT(DISTINCT icd.penyakit SEPARATOR "; ") as penyakit, GROUP_CONCAT(DISTINCT obat.nama_obat SEPARATOR "; ") as obat, GROUP_CONCAT(DISTINCT pelayanan_obat.dosis SEPARATOR "; ") as dosis, GROUP_CONCAT(DISTINCT pelayanan_obat.qty SEPARATOR "; ") as jml_obat, pelayanan.anamnesa',false);
        $sql->from('pelayanan');
		...
		$sql->join('pelayanan_obat','pelayanan.kd_trans_pelayanan=pelayanan_obat.kd_trans_pelayanan','left');
