	...
	public function MaxKodeRujukan($date,$kd_pus){
		...
		if($data->num_rows() > 0 ){
			foreach($data->result() as $t){
				...
				$hasil = '440'.'/'.sprintf("%04s", $tmp).'/'.$this->generate_romawi($pecah[1]).'/ '.$kd_pus.'-'.$pecah[0];
			} 
		}else{
			//$hasil = 'RJK-'.'0001/'.$kd_pus.'/'.$this->generate_romawi($pecah[1]).'/'.$pecah[0];
			$hasil = '440'.'/'.'0001/'.$this->generate_romawi($pecah[1]).'/ '.$kd_pus.'-'.$pecah[0];
		} 
...
/**********************************************************************************************************/
	...
	{
		$this->db->select('MAX(cast(substring(kd_rekam_medis from 13 for 6) as unsigned)) as no_rm');
		$this->db->from('pasien');
