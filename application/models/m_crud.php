...
public function ItemAwalApotek($id){
	...
	function get_all_bayar_tindakan() {
		...
		$this->db->where('btindakan_header.tgl_bayar',$curr_date);
	...
	function get_bayar_tindakan_by_id_pasien($id) {
		$this->db->select('btindakan_header.id_bayar,btindakan_header.tgl_bayar,btindakan_header.kd_bayar,btindakan_header.kd_rekam_medis,    btindakan_header.bayar,btindakan_header.kembalian,btindakan_header.nopelayanan,pasien.nm_lengkap,pasien.kd_rekam_medis');
		$this->db->from('btindakan_header');
		$this->db->join('pasien','btindakan_header.kd_rekam_medis = pasien.kd_rekam_medis','left');
		$this->db->where('btindakan_header.kd_bayar',$id);
		$query = $this->db->get();
		return $query->result_array();
...
/**********************************************************************************************************/
	...
	function get_sta_pelayanan_by_id()
    	{
        	$id = $this->uri->segment(4);
        	$kode['kd_trans_pelayanan'] = $id;
        	$up['kd_status_pasien']     = 'SKP-5';

        	$data = $this->m_crud->getSelectedData("pelayanan",$kode);
        	if($data->num_rows()>0){
        	$this->m_crud->updateData("pelayanan",$up,$kode);
        	}
    	}
	...
	{
		$this->db->select('MAX(cast(substring(kd_rekam_medis from 1 for 6) as unsigned)) as no_rm');
		$this->db->from('pasien');
    ...
    {
        $this->db->distinct();
        $this->db->select('pelayanan.kd_trans_pelayanan,pelayanan.tgl_pelayanan,jenis_layanan.jenis_layanan, pasien.nm_lengkap,
        ...
        $this->db->from('pelayanan');
        $this->db->join('pelayanan_obat', 'pelayanan.kd_trans_pelayanan = pelayanan_obat.kd_trans_pelayanan','inner');
        $this->db->join('unit_pelayanan', 'pelayanan.kd_unit_pelayanan = unit_pelayanan.kd_unit_pelayanan','inner');
        $this->db->join('jenis_layanan', 'pelayanan.kd_jenis_layanan = jenis_layanan.kd_jenis_layanan','inner');
        $this->db->join('pasien', 'pelayanan.kd_rekam_medis = pasien.kd_rekam_medis','inner');
        $this->db->where('pelayanan.tgl_pelayanan',date('Y-m-d'));
        $this->db->where('pelayanan.status !=','1');
	...
	function check_duplicate($rm, $kd_unit)
	{
		$this->db->select('count(*) as jumlah');	
		$this->db->from('pelayanan');
		$this->db->where('kd_rekam_medis', $rm);
		$this->db->where('tgl_pelayanan', date('Y-m-d'));
		$this->db->where('kd_unit_pelayanan', $kd_unit);
	...
	function get_total_kunjungan($kd_rekmed)
	{
		$this->db->select('count(*) as total');
		$this->db->from('pelayanan');
		$this->db->where('kd_rekam_medis', $kd_rekmed);
