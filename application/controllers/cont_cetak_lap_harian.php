	...
	function register_harian($par1 = '', $par2 = '', $par3 = '')
	{
		...
		if ($par1 == 'cetak') {
            ...
            foreach($pasien as $rs){
				...
				if($rs['kd_wil']==1){	$wil = "Dalam Wilayah";
				} else if ($rs['kd_wil']==2) {$wil="Luar Wilayah";
                } else { $wil="Luar Kota";}
				...
				$objPHPExcel->getActiveSheet()->setCellValue('E'.$i, $wil);
                $objPHPExcel->getActiveSheet()->setCellValue('F'.$i, $rs['alamat']);
				$objPHPExcel->getActiveSheet()->setCellValue('G'.$i, $rs['nm_kelurahan']);
				$objPHPExcel->getActiveSheet()->setCellValue('H'.$i, $rs['nm_kota']);
                $objPHPExcel->getActiveSheet()->setCellValue('I'.$i, $rs['gol_umur']);
                $objPHPExcel->getActiveSheet()->setCellValue('J'.$i, $rs['cara_bayar']);
				$objPHPExcel->getActiveSheet()->setCellValue('K'.$i, $asuransi);
				$objPHPExcel->getActiveSheet()->setCellValue('L'.$i, $rs['anamnesa']);
				$objPHPExcel->getActiveSheet()->setCellValue('M'.$i, $rs['penyakit']);
				$objPHPExcel->getActiveSheet()->setCellValue('N'.$i, $rs['tindakan']);
				$objPHPExcel->getActiveSheet()->setCellValue('O'.$i, $obatku);
				$objPHPExcel->getActiveSheet()->setCellValue('P'.$i, $rs['jns_kasus']);
                $objPHPExcel->getActiveSheet()->setCellValue('Q'.$i, $rs['keterangan']);
                $objPHPExcel->getActiveSheet()->setCellValue('R'.$i, $rs['nm_dokter']);
				$objPHPExcel->getActiveSheet()->setCellValue('S'.$i, $rs['nm_unit']);
                $i++;
                $no++;
            }
