...
class Cont_transaksi_pelayanan extends CI_Controller
{
	...
	function pelayanan_today($par1 = '', $par2 = '', $par3 = '')
	{
		...
		if ($par1 == 'tambah') {
			...
			if($this->input->post('kd_status_pasien') == "SKP-4")	{
				$pelayanan['no_rujukan'] 			= $this->m_crud->MaxKodeRujukan(date('Y-m-d'),'PkmBotim');
			} else {
			...
			// cek kunjungan
			$total_kunjungan = $this->m_crud->get_total_kunjungan($pelayanan['kd_rekam_medis']);
			if($total_kunjungan == 0)
				$pelayanan['kunjungan'] = 1; // baru
			else
				$pelayanan['kunjungan'] = 2; // lama
			#print_r($pelayanan_penyakit); exit;

			// start transaksi dengan db
			$this->db->trans_start();
			
			#Secho '<pre>'; print_r($pelayanan); exit;
			
			$cek_duplicate = $this->m_crud->check_duplicate($pelayanan['kd_rekam_medis'], $pelayanan['kd_unit_pelayanan']);
			#print $this->db->last_query(); exit;
			#print_r($cek_duplicate); exit;
			if($cek_duplicate['jumlah'] > 0){
				$this->db->trans_complete();
				$this->session->set_flashdata('flash_message', 'Data transaksi pelayanan dengan nama '.$this->input->post('nm_lengkap').' sudah di input, mohon periksa kembali di daftar pelayanan!');	
		...
		if ($par1 == 'ubah' && $par2 == 'do_update') {
			...
			if ($this->session->userdata('id_akses') == 2) // pendaftaran hanya update 3 field 
			{
				...
				if ($data_lama[0]['kd_unit_pelayanan'] != ($this->input->post('kd_unit_pelayanan'))) {
					$pelayanan['no_antrian'] = $this->m_crud->generate_queue($this->input->post('kd_unit_pelayanan'));	
				} else { 
				$pelayanan['no_antrian'] = $data_lama[0]['no_antrian'];
				}
				...
				// update rujukan
				if($pelayanan['kd_status_pasien']=="SKP-1" || $pelayanan['kd_status_pasien']=="SKP-2"){
					$pelayanan['no_rujukan'] 		= "";
					$pelayanan['jenis_rujukan'] 	= "";
					$pelayanan['tempat_rujukan']	= "";
					$pelayanan['poli_rujukan']		= "";

				} elseif($pelayanan['kd_status_pasien']=="SKP-3") {
					// poli
					$pelayanan['no_rujukan'] 		= "";
					$pelayanan['jenis_rujukan'] 	= "";
					$pelayanan['tempat_rujukan']	= "";

					$poli_rujukan					= $this->m_crud->get_unit_pelayanan_by_id($this->input->post('poli_rujukan'));;
					$pelayanan['poli_rujukan']		= $poli_rujukan['nm_unit'];

					$trans_id 						= $this->m_crud->generate_transaksi();
					$rujukan['kd_trans_pelayanan'] 	= $trans_id;

					$rujukan['no_antrian']			= $this->m_crud->generate_queue($this->input->post('poli_rujukan'));
					$rujukan['kd_rekam_medis']		= $data_lama[0]['kd_rekam_medis'];
					$rujukan['tgl_pelayanan'] 	= date('Y-m-d');

					$rujukan['kd_unit_pelayanan'] 	= $this->input->post('poli_rujukan');
					$rujukan['kd_jenis_layanan'] 	= $this->input->post('kd_jenis_layanan');


					if($rujukan['kd_jenis_layanan'] == 3){
						$rujukan['kd_bed']		 		= $this->input->post('kd_bed');
					} else {
						$rujukan['kd_bed']		 		= NULL;
					}

					$rujukan['kd_bayar'] 		= $this->input->post('kd_bayar');
					$rujukan['kd_puskesmas']	= $data_lama[0]['kd_puskesmas'];
					$rujukan['umur']			= $data_lama[0]['umur'];
					$rujukan['kd_gol_umur']		= $data_lama[0]['kd_gol_umur'];
					$rujukan['status']			= "";
					$rujukan['timestamps']	 	= date('Y-m-d H:i:s');

					$this->m_crud->simpan('pelayanan', $rujukan);

				} elseif($pelayanan['kd_status_pasien']=="SKP-4") {
					// rs
					if($data_lama[0]['no_rujukan'] == "" || $data_lama[0]['no_rujukan'] == null)
						$pelayanan['no_rujukan']	= $this->m_crud->MaxKodeRujukan(date('Y-m-d'),'PkmBotim');
					else
						$pelayanan['no_rujukan']	= $data_lama[0]['no_rujukan'];
			...
			$data['counter3']	   	= $this->m_crud->get_total_pelayanan_obat_by_id($par2);
			 ...
			 $data['sta_pelayanan']	= $this->m_crud->get_sta_pelayanan_by_id($par2);
	...
	/*	if ($this->session->userdata('id_akses') == 2) // pendaftaran, dengan NO KK dan no KS
		...
		if ($this->session->userdata('id_akses') == 2) // pendaftaran, dengan NO KK dan no KS
		{ $this->table->set_heading('No Layanan','No Antrian','No. RM','Nama Pasien','Alamat','Kelurahan','Umur','Pembayaran','No. KS','Status', 'Aksi');
		} 
		...
		{ 	// view datatable jika di poli
			$this->table->set_heading('No Layanan','No Antrian','No. RM','Nama Pasien','Alamat','Umur','Status', 'Aksi');
		} else { $this->table->set_heading('No Layanan','No Antrian','No. RM','Nama Pasien','Alamat','Umur','Pembayaran','Status', 'Aksi'); 
		}	
...
/***MASTER PELAYANAN***/
	...
	function pelayanan_today_lab($par1 = '', $par2 = '', $par3 = '')
	{
		...
		if ($par1 == 'tambah') {
			...
			//----------- cek no rujukan
			if( $this->input->post('kd_status_pasien') == "SKP-3" || $this->input->post('kd_status_pasien') == "SKP-4")	{
				$pelayanan['no_rujukan'] 			= $this->m_crud->MaxKodeRujukan(date('Y-m-d'),'PkmBoteng');
			} else {
				$pelayanan['no_rujukan'] 			= NULL;
			}
...
/***AUTO COMPLETE***/
	...
	public function cetak_resep()
	{
			...
			$text = "SELECT pelayanan.kd_trans_pelayanan,pelayanan.kd_rekam_medis,pelayanan.tgl_pelayanan,pelayanan.kd_jenis_layanan,pelayanan.kd_unit_pelayanan,
				...
				pasien.no_reg,pasien.nm_lengkap,pasien.alamat,pasien.kd_kota,pasien.kd_bayar,cara_bayar.cara_bayar,dokter.nm_dokter,unit_pelayanan.nm_unit
				FROM pelayanan
				LEFT JOIN pasien ON pelayanan.kd_rekam_medis = pasien.kd_rekam_medis
				LEFT JOIN cara_bayar ON pasien.kd_bayar = cara_bayar.kd_bayar
				INNER JOIN dokter ON pelayanan.kd_dokter = dokter.kd_dokter
			...
			$text = "SELECT
                    ...
                    satuan_kecil.sing_sat_kecil_obat
                    FROM
                    pelayanan_obat
...
$html .='<table width="100%" border="0" cellpadding="3">
  ...
  <tr>
    ...
    public function cetak_antrian()
    {
        ...
        $text = "SELECT pelayanan.kd_trans_pelayanan,pelayanan.no_antrian,pelayanan.tgl_pelayanan,pelayanan.kd_bayar,
                cara_bayar.cara_bayar,pelayanan.kd_puskesmas,set_puskesmas.nm_puskesmas,set_puskesmas.alamat,set_puskesmas.kd_kota,
                set_puskesmas.telp,set_puskesmas.logo,pelayanan.kd_unit_pelayanan,unit_pelayanan.nm_unit,kota.nm_kota,set_puskesmas.kd_propinsi,
                propinsi.nm_propinsi FROM  pelayanan
                LEFT JOIN cara_bayar ON pelayanan.kd_bayar = cara_bayar.kd_bayar
                LEFT JOIN set_puskesmas ON pelayanan.kd_puskesmas = set_puskesmas.kd_puskesmas
                LEFT JOIN unit_pelayanan ON pelayanan.kd_unit_pelayanan = unit_pelayanan.kd_unit_pelayanan
                LEFT JOIN kota ON set_puskesmas.kd_kota = kota.kd_kota
                LEFT JOIN propinsi ON set_puskesmas.kd_propinsi = propinsi.kd_propinsi
                WHERE pelayanan.kd_trans_pelayanan='$id'";
        $hasil = $this->m_crud->manualQuery($text);
        foreach($hasil ->result() as $t){
            $d['tgl_pelayanan']    = $this->m_crud->tgl_indo($t->tgl_pelayanan);
            $d['nm_lengkap']    = $t->nm_lengkap;
            $d['no_antrian']    = $t->no_antrian;
            $d['nm_puskesmas']  = $t->nm_puskesmas;
            $d['cara_bayar']    = $t->cara_bayar;
            $d['alamat']        = $t->alamat;
            $d['nm_unit']       = $t->nm_unit;
        ...
        $this->template->tampil_cetak_antrian('cetak_antrian',$d);
