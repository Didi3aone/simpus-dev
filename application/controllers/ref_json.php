...
class Ref_json extends CI_Controller 
{
	...
	public function CariBarang()
	{
		if($this->session->userdata('logged_in')!="")
			...
			WHERE obat.kd_obat='$kode' GROUP BY kd_obat LIMIT 200";
	...
	public function CariObat()
	{
		if($this->session->userdata('logged_in')!="")
			...
			$text = "SELECT obat.*, satuan_kecil.sat_kecil_obat FROM obat INNER JOIN satuan_kecil ON obat.kd_sat_kecil_obat = satuan_kecil.kd_sat_kecil_obat WHERE obat.kd_obat='$kode' GROUP BY kd_obat LIMIT 200";
			$d = $this->m_crud->manualQuery($text);
			$row = $d->num_rows();
			if($row>0){
				foreach($d->result() as $t){
	...
	public function CariTindakan()
	{
		if($this->session->userdata('logged_in')!="")
			...
			$text = "SELECT * FROM tindakan WHERE kd_produk='$kode' GROUP BY kd_produk LIMIT 200";
			$d = $this->m_crud->manualQuery($text);
			$row = $d->num_rows();
			if($row>0){
				foreach($d->result() as $t){
    ...
    public function InfoUnitFarmasi()
    {
        ...
        FROM unit_farmasi WHERE unit_farmasi.kd_unit_farmasi = '$kode' GROUP BY unit_farmasi.kd_unit_farmasi LIMIT 200";
        $d = $this->m_crud->manualQuery($text);
        $row = $d->num_rows();
        if($row>0){
            foreach($d->result() as $t){
	...
	public function CariPasien()
	{
		if($this->session->userdata('logged_in')!="")
			...
			$text = "SELECT * FROM pasien WHERE kd_rekam_medis='$koderek' GROUP BY kd_rekam_medis LIMIT 200";
			$d = $this->m_crud->manualQuery($text);
			$row = $d->num_rows();
			if($row>0){
				foreach($d->result() as $t){
...
//////////////////////////////////////* MENAMPILKAN DATA BARANG *///////////////////////////////////////	
	public function DataBarang()
	{
		if($this->session->userdata('logged_in')!="")
			...
			if(empty($cari)){
				$text = "SELECT * FROM obat LIMIT 200";
			}else{
				$text = "SELECT * FROM obat WHERE kd_obat LIKE '%$cari%' OR nama_obat LIKE '%$cari%' LIMIT 200";
			}
...
//////////////////////////////////////* MENAMPILKAN DATA PASIEN *///////////////////////////////////////		
	public function DataPasien()
	{
		if($this->session->userdata('logged_in')!="")
			...
			if(empty($caripasien)){
				$text = "SELECT * FROM pasien LIMIT 200";
			}else{
				$text = "SELECT * FROM pasien WHERE kd_rekam_medis LIKE '%$caripasien%' OR nm_lengkap LIKE '%$caripasien%' LIMIT 200";
			}
			$d['data'] = $this->m_crud->manualQuery($text);
	...
	public function DataTindakan()
	{
		if($this->session->userdata('logged_in')!="")
			...
			if(empty($caritindakan)){
				$text = "SELECT * FROM tindakan LIMIT 200";
			}else{
				$text = "SELECT * FROM tindakan WHERE kd_produk LIKE '%$caritindakan%' OR produk LIKE '%$caritindakan%' LIMIT 200";
			}
			$d['data'] = $this->m_crud->manualQuery($text);
		...
		if($this->session->userdata('logged_in')!="")
			...
			if(empty($cari)){
                $text = "SELECT DISTINCT
                    pelayanan.kd_trans_pelayanan,pelayanan.tgl_pelayanan,jenis_layanan.jenis_layanan, pasien.nm_lengkap,
                    unit_pelayanan.kd_antrian,pasien.tanggal_daftar,pasien.tempat_lahir,pasien.tanggal_lahir,pasien.alamat,pasien.kd_rekam_medis, pelayanan.umur
                    FROM pelayanan
                    INNER JOIN pasien ON pelayanan.kd_rekam_medis = pasien.kd_rekam_medis
                    ...
                    INNER JOIN pelayanan_obat ON pelayanan.kd_trans_pelayanan = pelayanan_obat.kd_trans_pelayanan
                    WHERE  DATE_FORMAT(pelayanan.tgl_pelayanan,'%y-%b-%d') = DATE_FORMAT(NOW(),'%y-%b-%d')
                    AND pelayanan.status <> '1' ORDER BY pelayanan.kd_trans_pelayanan DESC LIMIT 200";
			}else{
				$text = "SELECT
                        pelayanan.kd_trans_pelayanan,pelayanan.tgl_pelayanan,jenis_layanan.jenis_layanan,pasien.nm_lengkap,
                        unit_pelayanan.nm_unit,pasien.tanggal_daftar,pasien.tempat_lahir,pasien.tanggal_lahir
                        FROM pelayanan
                        INNER JOIN pasien ON pelayanan.kd_rekam_medis = pasien.kd_rekam_medis
                        INNER JOIN jenis_layanan ON pelayanan.kd_jenis_layanan = jenis_layanan.kd_jenis_layanan
                        INNER JOIN unit_pelayanan ON pelayanan.kd_unit_pelayanan = unit_pelayanan.kd_unit_pelayanan
	...
	public function DataObat()
	{
		if($this->session->userdata('logged_in')!="")
			...
			if(empty($cariobat)){
				...
				$text = "SELECT * FROM obat WHERE kd_obat LIKE '%$cariobat%' OR nama_obat LIKE '%$cariobat%' LIMIT 200";
			}
			$d['data'] = $this->m_crud->manualQuery($text);
	...
	public function DataRekamMedis()
	{
		if($this->session->userdata('logged_in')!="")
			...
			if(empty($cari)){
				...
				$text = "SELECT * FROM pasien WHERE kd_rekam_medis LIKE '%$cari%' OR nm_lengkap LIKE '%$cari%' LIMIT 200";
			}
			$d['data'] = $this->m_crud->manualQuery($text);
