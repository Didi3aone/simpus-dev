...
//            $this->template->display_login();
			...
			switch($this->session->userdata('id_akses')){
				...
				case 14:
					...
					break;
				case 15: //gizi
					redirect('klinik_gizi/dashboard');
					break;
				case 16: //lansia
					redirect('poli_lansia/dashboard');
					break;
				case 17: // promkes
					redirect('poli_lansia/dashboard');
					break;
...
//			'email' => $this->input->post('email'),
		...
		if ($query->num_rows() > 0) {
			...
			if ($row ->id_akses == 1) {
			 ...
			 else if ($row ->id_akses == 15) {
			
				$this->session->set_userdata('akses', 'klinik_gizi');
				$this->session->set_userdata('kd_unit_pelayanan', '10');
			}	
			else if ($row ->id_akses == 16) {
			
				$this->session->set_userdata('akses', 'poli_lansia');
				$this->session->set_userdata('kd_unit_pelayanan', '24');
			}																
			else if ($row ->id_akses == 17) {
			
				$this->session->set_userdata('akses', 'poli_promkes');
				$this->session->set_userdata('kd_unit_pelayanan', '25');
			}	
