...
class Barang extends CI_Controller
{
	...
	public function cetak_keluar_apotek()
	{
			...
			$text = "SELECT brg_apotek_keluar_header.id_keluar,brg_apotek_keluar_header.kd_keluar,brg_apotek_keluar_header.tgl_keluar,brg_apotek_keluar_header.keterangan,
				brg_apotek_keluar_header.kd_unit_farmasi,brg_apotek_keluar_header.kd_trans_pelayanan,pasien.nm_lengkap,pasien.alamat,
				pasien.no_hp,pasien.kd_bayar,pasien.no_reg,kecamatan.nm_kecamatan,kota.nm_kota,pasien.kd_pos,pelayanan.umur,pelayanan.kd_dokter,
				dokter.nm_dokter,pelayanan.kd_unit_pelayanan,unit_pelayanan.nm_unit, pelayanan.kd_bayar,cara_bayar.cara_bayar FROM
				...
				INNER JOIN cara_bayar ON pelayanan.kd_bayar = cara_bayar.kd_bayar
		...
		$d['cara_bayar']        = $t->cara_bayar;
            ...
            brg_apotek_keluar_header.nm_lengkap,
            ...
            FROM
            brg_apotek_keluar_header
            INNER JOIN pelayanan_obat ON brg_apotek_keluar_header.kd_trans_pelayanan = pelayanan_obat.kd_trans_pelayanan
            INNER JOIN obat ON pelayanan_obat.kd_obat = obat.kd_obat
            INNER JOIN satuan_kecil ON obat.kd_sat_kecil_obat = satuan_kecil.kd_sat_kecil_obat
            WHERE brg_apotek_keluar_header.kd_keluar='$id'";
        ...
        $text = "SELECT set_puskesmas.`status`,set_puskesmas.kd_puskesmas,set_puskesmas.nm_puskesmas,set_puskesmas.alamat,
                    ...
                    set_puskesmas.telp,set_puskesmas.logo,kecamatan.nm_kecamatan,kota.nm_kota,kelurahan.nm_kelurahan,propinsi.nm_propinsi
                    FROM set_puskesmas
                    LEFT JOIN kecamatan ON kecamatan.kd_kecamatan = set_puskesmas.kd_kecamatan
                    INNER JOIN kota ON kota.kd_kota = set_puskesmas.kd_kota
                    INNER JOIN kelurahan ON kelurahan.kd_kelurahan = set_puskesmas.kd_kelurahan
                    INNER JOIN propinsi ON propinsi.kd_propinsi = set_puskesmas.kd_propinsi";
        $hasil = $this->m_crud->manualQuery($text);
	...
	public function lihat_apotek_out()
	{
		if($this->session->userdata('logged_in')!="")
		{
            ...
            $text = "SELECT
                    ...
                    $where ORDER BY brg_apotek_keluar_header.tgl_keluar";			
			$d['data'] = $this->db->query($text);
